...
import style from '@editor/ui/template/style';
import standardTheme from '@editor/ui/theme/standard';
import { styleLoad } from '@editor/util';

import icon from '@editor-svg/default.svg';
  ...
  _loadDefaultSvgIcon() {
    if (!document.getElementById('tui-image-editor-svg-default-icons')) {
      ...
      if (icon.indexOf('base64,') > -1) {
        ...
        svgText = atob(encodedURI);
      } else {
        // Vite might have loaded it as a raw string or data URI with percent encoding
        svgText = decodeURIComponent(icon.replace(/data:image\/svg\+xml,/, ''));
        // If it still looks like a data URI or filename, we might need closer inspection,
        // but usually Vite's raw loader or similar will give us the string.
        if (svgText.indexOf('<svg') === -1 && icon.indexOf('<svg') > -1) {
          svgText = icon;
        }
      }
      const dom = parser.parseFromString(svgText, 'text/xml');
